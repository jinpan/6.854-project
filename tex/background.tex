...
90's with Leighton et. al [CITE]. %\cite{leighton}
...
state-of-the-art linear program solvers [CITE] %\cite{goldberg}.
The main idea of how
these early FPAS' work is a rerouting method, generalizing on
fractional packing techniques[CITE-karger abstract] %\cite{karger}.
Initially, the
...
problem [CITE]%\cite{karger}. 
Since then, these bounds have since been increased to
$\~{O}(\omega^{-2}m^2)$ in 2000[Cite]%\cite{grigoriadis}. 
...
and an $O(m^{o(1)})$-competitive oblivious routing scheme [cite].%\cite{almostLinear}
